package com.mindberry.javaapp.web.rest;
...
import com.mindberry.javaapp.config.KafkaProperties;
...
class MyBackendAppKafkaResourceIT {
    ...
    void setup() {
        ...
        MyBackendAppKafkaResource kafkaResource = new MyBackendAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/my-backend-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/my-backend-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
